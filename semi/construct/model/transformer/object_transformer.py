...
from construct.model.group_modules import GConv2d
from construct.utils.tensor_utils import aggregate
from construct.model.transformer.positional_encoding import PositionalEncoding
from construct.model.transformer.transformer_layers import *
from construct.model.transformer.trajectory_attention import *
...
class QueryTransformerBlock(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        #self.read_from_pixel = TrajectoryAwareCrossAttention(self.embed_dim,
...
if __name__ == '__main__':
    ...
    # 直接传入 cfg (它本身就是 model_cfg)
    model = QueryTransformer(cfg).cuda()
    ...
    pixel_readout = torch.randn(batch_size, num_objects, embed_dim, h, w).cuda()
    obj_memory = torch.randn(batch_size, num_objects, T, num_queries, embed_dim + 1).cuda()

    # 前向传播
    output, aux = model(pixel_readout, obj_memory)
    #print(model)

    print(f"Input pixel_readout shape: {pixel_readout.shape}")
    print(f"Input obj_memory shape: {obj_memory.shape}")
    print(f"Output shape: {output.shape}")
    print(f"Aux keys: {aux.keys() if aux else None}")
