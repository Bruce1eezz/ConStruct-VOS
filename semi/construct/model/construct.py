...
from construct.model.modules import *
from construct.model.big_modules import *
from construct.model.aux_modules import AuxComputer
from construct.model.atcf import CurrentFrameEnhancementModule
from construct.model.utils.memory_utils import *
from construct.model.transformer.object_transformer import QueryTransformer
from construct.model.transformer.object_summarizer import ObjectSummarizer
from construct.utils.tensor_utils import aggregate
...
class Construct(nn.Module):
    def __init__(self, cfg: DictConfig, *, single_object=False):
        ...
        self.pixel_encoder = PixelEncoder_DINOv3_convnext(model_cfg) # new: use dinov3_convnext_tiny
        ...
        self.temporal_refiner_16 = CurrentFrameEnhancementModule(self.ms_dims[0],
                                                                 self.ms_dims[0],
                                                                 sequence_length=self.temporal_window)
        self.temporal_refiner_8 = CurrentFrameEnhancementModule(self.ms_dims[1],
                                                                self.ms_dims[1],
                                                                sequence_length=self.temporal_window)
    ...
    def _temporal_window_indices(self, length: int, center: int):
        radius = self.temporal_window // 2
        return [min(max(center + offset, 0), length - 1) for offset in range(-radius, radius + 1)]

    def _apply_temporal_context(self, feature_seq: torch.Tensor,
                                module: CurrentFrameEnhancementModule) -> torch.Tensor:
        outputs = []
        seq_length = feature_seq.shape[1]
        for ti in range(seq_length):
            window = feature_seq[:, self._temporal_window_indices(seq_length, ti)]
            outputs.append(module(window))
        return torch.stack(outputs, dim=1)

    def refine_temporal_features(self, ms_features: List[torch.Tensor]):
        squeeze_time = ms_features[0].dim() == 4
        if squeeze_time:
            ms_features = [feat.unsqueeze(1) for feat in ms_features]

        refined = [
            self._apply_temporal_context(ms_features[0], self.temporal_refiner_16),
            self._apply_temporal_context(ms_features[1], self.temporal_refiner_8),
            ms_features[2],
        ]

        if squeeze_time:
            refined = [feat[:, 0] for feat in refined]
            pix_feat = self.pix_feat_proj(refined[0])
        else:
            batch_size, seq_length = refined[0].shape[:2]
            pix_feat = self.pix_feat_proj(refined[0].flatten(start_dim=0, end_dim=1))
            pix_feat = pix_feat.view(batch_size, seq_length, *pix_feat.shape[1:])

        return refined, pix_feat
