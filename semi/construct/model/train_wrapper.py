...
class ConstructTrainWrapper(Construct):
    ...
    def forward(self, data: Dict):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            ms_feat, _ = self.encode_image(frames_flat)
            ms_feat = [self.move_t_out_of_batch(f) for f in ms_feat]
            ms_feat, pix_feat = self.refine_temporal_features(ms_feat)
            with torch.cuda.amp.autocast(enabled=False):
                keys, shrinkages, selections = self.transform_key(
                    ms_feat[0].flatten(start_dim=0, end_dim=1).float())

            # ms_feat: tuples of B*T*C*H*W -> B*T*C*H*W after temporal refinement
