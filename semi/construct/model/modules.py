...
from construct.model.freq_fuse import FreqFusion
...
class MaskUpsampleBlock(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, scale_factor: int = 2):
        super().__init__()
        if scale_factor != 2:
            raise ValueError('MaskUpsampleBlock only supports scale_factor=2 with FreqFusion')
        self.lr_proj = GConv2d(in_dim, out_dim, kernel_size=1)
        self.freq_fuse = FreqFusion(out_dim, out_dim)
        self.out_conv = GroupResBlock(out_dim * 2, out_dim)
        self.scale_factor = scale_factor

    def forward(self, in_g: torch.Tensor, skip_f: torch.Tensor) -> torch.Tensor:
        batch_size, num_objects = in_g.shape[:2]
        skip_f = skip_f.unsqueeze(1).expand(-1, num_objects, -1, -1, -1)
        hr_feat = skip_f.flatten(start_dim=0, end_dim=1)
        lr_feat = self.lr_proj(in_g).flatten(start_dim=0, end_dim=1)
        _, fused_hr, fused_lr = self.freq_fuse(hr_feat=hr_feat, lr_feat=lr_feat)
        fused = torch.cat([fused_hr, fused_lr], dim=1)
        fused = fused.view(batch_size, num_objects, *fused.shape[1:])
