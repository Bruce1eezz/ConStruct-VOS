...
from construct.model.train_wrapper import ConstructTrainWrapper
from construct.model.utils.parameter_groups import get_parameter_groups
from construct.model.losses import LossComputer
from construct.utils.log_integrator import Integrator
from construct.utils.image_saver import vis
from construct.utils.logger import TensorboardLogger
from construct.utils.time_estimator import TimeEstimator
...
class Trainer:
    def __init__(self, cfg: DictConfig, stage_cfg: DictConfig, log: TensorboardLogger, run_path):
        ...
        self.construct = nn.parallel.DistributedDataParallel(ConstructTrainWrapper(cfg, stage_cfg).cuda(),
        ...
        self.log.log_string('model_size',
                            str(sum([param.nelement() for param in self.construct.parameters()])))
        self.log.log_string(
            'number_of_parameters_that_require_gradient',
            str(
                sum([
                    param.nelement()
                    for param in filter(lambda p: p.requires_grad, self.construct.parameters())
                ])))
        ...
        self.train()
        parameter_groups = get_parameter_groups(self.construct, stage_cfg, print_log=(local_rank == 0))
    ...
    def do_pass(self, data, it=0):
        ...
        if self._is_train:
            if it % self.log_text_interval == 0 and it != 0:
                self.train_integrator.add_tensor('lr', self.scheduler.get_last_lr()[0]) # backbone lr
        ...
        if self.use_amp:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(self.construct.parameters(), self.clip_grad_norm)
    ...
    def save_weights(self, it, save_copy=False):
        ...
        if save_copy:
            ...
            torch.save(self.construct.module.state_dict(), model_path)
    ...
    def save_checkpoint(self, it, save_copy=False):
        ...
        checkpoint = {
            'it': it,
            'weights': self.construct.module.state_dict(),
    ...
    def load_checkpoint(self, path):
        ...
        map_location = 'cuda:%d' % self.local_rank
        self.construct.module.load_state_dict(weights)
    ...
    def load_weights_in_memory(self, src_dict):
        self.construct.module.load_weights(src_dict)
    ...
    def weights(self):
        return self.construct.module.state_dict()
