...
from construct.model.group_modules import *
from construct.model.utils import resnet
from construct.model.modules import *
from construct.model.mose_plus import ProtoExpert, VisionMOE


class PixelEncoder_DINOv3_convnext(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        super().__init__()

        self.is_convnext = 'convnext' in model_cfg.pixel_encoder.type
        REPO_DIR = './dinov3'
        if self.is_convnext:
            if model_cfg.pixel_encoder.type == 'convnext_tiny':
                version = 'dinov3_convnext_tiny'
                weight_path = './dinov3/pretrained_weights/dinov3_convnext_tiny_pretrain_lvd1689m-21b726bb.pth'
                network = torch.hub.load(REPO_DIR, version, source='local', weights=weight_path)

                self.stem = network.downsample_layers[0]  # 4x4 conv, stride=4 -> 1/4

                # 1/4
                self.stage1 = network.stages[0]  # 3 blocks, channel = 96

                # 1/8
                self.downsample1 = network.downsample_layers[1]  # 2x2 conv, stride=2
                self.stage2 = network.stages[1]  # 3 blocks, channel = 192

                # 1/16
                self.downsample2 = network.downsample_layers[2]  # 2x2 conv, stride=2
                self.stage3 = network.stages[2]  # 9 blocks, channel = 384

                f16_dim = model_cfg.pixel_encoder.ms_dims[0]
                self.mose_f16 = VisionMOE(channels=f16_dim,
                                          proto_expert=ProtoExpert(hidden_size=f16_dim),
                                          num_experts=8,
